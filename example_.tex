...
{
  name={Jason agent},
  description={The agent program controlling the agent figure.}
}
...
{
  name={screen},
  description={Refers to the mid-air suspended canvas used as a screen for the installation. Functions as a window to the virtual space of the agent figures. Illuminated from the back by a projector.}
}
\newglossaryentry{live}
{
  name={live},
  description={Also live state. Refers to the agent's state, in which it mirrors the movements of the visitor it was assigned to.}
}
\newglossaryentry{island system}
{
  name={Island System},
  description={A concept similar to finite-state machines, used to visualize and model Jason agents. Described more closely in \autoref{chap:finalapproach}.}
}
...
\begin{figure}[h]
	...
	\caption{Picture of the agent figure mirroring the visitor’s movements (Picture taken by Günter Richard Wett)}
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Visitors with their assigned skeletons drawn onto them}
...
In case at least one of the visitors waves back, the figures express joy by clapping their hands, before switching back to live, mirroring the visitors’ actions indefinitely. 
